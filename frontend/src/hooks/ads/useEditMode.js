...
import { extractObjectId, findIdInObject } from "../../utils/business-logic/wizardUtils";
import { convertCountryCodesToNames, convertLocaleIdToLanguageCode } from "../../utils/formatters/locationUtils";
import { convertFacebookTypeToCTA } from "../../utils/formatters/ctaUtils";
import { parseGeoLocationsToFrontend } from "../../utils/parsers/locationParseUtils";
import { parseFlexibleSpecToFrontend } from "../../utils/parsers/targetingParseUtils";
        ...
        if (creativeIds.length > 0) {
          ...
          const creativesPromises = creativeIds.map(id =>
            axiosInstance.get("/api/creatives/database", {
              ...
              console.warn(`Failed to fetch creative ${id}:`, err.message);
              return null;
            })
          );
