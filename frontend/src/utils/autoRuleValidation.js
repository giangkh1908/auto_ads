 ...
 */
export const validateAutoRule = (formData) => {
  const errors = [];
  ...
  if (!formData.conditions || formData.conditions.length === 0) {
    errors.push("Vui lòng thêm ít nhất một điều kiện");
  } else {
    ...
    if (validConditions.length === 0) {
      errors.push("Vui lòng nhập giá trị cho ít nhất một điều kiện");
    }

    // Validate each condition value is a number
    formData.conditions.forEach((condition, index) => {
      if (condition.value !== "" && condition.value !== null) {
        const value = parseFloat(condition.value);
        if (isNaN(value)) {
          errors.push(
            `Giá trị điều kiện "${condition.metric}" (điều kiện ${index + 1}) không hợp lệ`
          );
        }
      }
    });
...
export const validateAndConvertToBackend = (
  ...
  // Validate conditions
  if (!formData.conditions || formData.conditions.length === 0) {
    throw new Error("Vui lòng thêm ít nhất một điều kiện");
  }
  ...
  const conditions = formData.conditions
    ...
    .map((condition) => {
      const value = parseFloat(condition.value);
      if (isNaN(value)) {
        throw new Error(`Giá trị điều kiện "${condition.metric}" không hợp lệ`);
      }
      return convertConditionToBE(condition);
    });

  if (conditions.length === 0) {
    throw new Error("Vui lòng nhập giá trị cho ít nhất một điều kiện");
  }
