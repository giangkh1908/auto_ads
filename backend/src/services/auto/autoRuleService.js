...
export async function evaluateConditions(rule) {
  try {
    ...
    accountIdVariants.add(String(externalAccountId));
    if (normalizedAccountId && normalizedAccountId !== externalAccountId) {
      accountIdVariants.add(String(normalizedAccountId));
    }

    console.log(
      `[AutoRule Evaluate] External Account ID: ${externalAccountId} → Variants for matching: ${Array.from(
        accountIdVariants
      ).join(", ")}`
    );
    ...
    // Lấy external IDs tương ứng để match với AdPerformance
    const orConditions = [];
    ...
    if (campaign_ids && campaign_ids.length > 0) {
      ...
      const externalCampaignIds = campaigns
        .map((c) => c.external_id)
        .filter((id) => !!id);

      console.log(
        `[AutoRule Evaluate] Mapped campaign_ids → external_campaign_id: ${externalCampaignIds.join(
          ", "
        )}`
      );

      if (externalCampaignIds.length > 0) {
        orConditions.push({ external_campaign_id: { $in: externalCampaignIds } });
      }
    }

    // Adsets → external_adset_id
    if (adset_ids && adset_ids.length > 0) {
      ...
      const externalAdsetIds = adsets
        .map((s) => s.external_id)
        .filter((id) => !!id);

      console.log(
        `[AutoRule Evaluate] Mapped adset_ids → external_adset_id: ${externalAdsetIds.join(", ")}`
      );

      if (externalAdsetIds.length > 0) {
        orConditions.push({ external_adset_id: { $in: externalAdsetIds } });
      }
    }
    ...
    if (ad_ids && ad_ids.length > 0) {
      ...
      const externalAdIds = ads
        .map((a) => a.external_id)
        .filter((id) => !!id);

      console.log(
        `[AutoRule Evaluate] Mapped ad_ids → external_ad_id: ${externalAdIds.join(", ")}`
      );

      if (externalAdIds.length > 0) {
...
export async function processRule(rule) {
  ...
  try {
    ...
    // Chuẩn bị dữ liệu update cho AutomationRule
    const updateSet = {
      last_run_at: now,
      last_error: null,
      last_error_at: null,
    ...
    if (conditionsMet) {
      ...
      updateSet.last_triggered_at = now;
      updateSet.status = "TRIGGERED";
      updateInc.trigger_count = 1;
    ...
    updateSet.next_run_at = nextRunAt;
    ...
    // Save rule bằng updateOne để tránh VersionError do optimistic concurrency
    const updatePayload = {
      $set: updateSet,
      $inc: updateInc,
    };

    await AutomationRule.updateOne({ _id: rule._id }, updatePayload);
    ...
    // Save rule với error info, tránh VersionError
    await AutomationRule.updateOne({ _id: rule._id }, updatePayload);
