import Lead from "../../models/admin/lead.model.js";
import Note from "../../models/admin/note.model.js";
import validator from "validator";
import mongoose from "mongoose";

// Create New Lead, Form Public
export const createLead = async (req, res) => {
  try {
    ...
    // Check if phone already exists (not deleted)
    ...
    // Handle duplicate phone (unique index)
    if (error.code === 11000) {
      return res.status(400).json({
        success: false,
        message: "Số điện thoại này đã được đăng ký",
...
// Get Leads List (need authenticate - only CS Staff)
export const getLeads = async (req, res) => {
  try {
    ...
    // Build filter
    const filter = {
      deleted_at: null, // Only get leads that are not deleted
    };
    ...
    // Group notes by lead_id and get latest note (save note object to have _id)
    const notesMap = new Map();
    notes.forEach((note) => {
      const leadId = note.target_id.toString();
      if (!notesMap.has(leadId)) {
        notesMap.set(leadId, note); // Save note object instead of only note text
      }
    });
...
export const updateLeadStatus = async (req, res) => {
  try {
    ...
    // Find lead to check assigned status
    ...
    // Check: only user who assigned to lead can update status
    if (existingLead.assigned_to) {
      ...
      // If lead is not assigned, do not allow to update status
      return res.status(403).json({
        success: false,
...
export const assignLead = async (req, res) => {
  try {
    ...
    // If trying to assign lead (assigned_to has value) and lead is already assigned to another user
    if (assigned_to && existingLead.assigned_to) {
      ...
      // If trying to assign to another user (not the current user who is assigned)
      if (existingAssignedId !== newAssignedId) {
        return res.status(400).json({
          success: false,
          message: "Lead này đã được gán cho người khác. Vui lòng làm mới trang để xem thông tin mới nhất.",
        });
      }
    }
